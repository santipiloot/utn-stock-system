...
async def crear_usuario(usuario: UsuariosIn):
    query = "INSERT INTO usuarios (nombre, username, contrasena) VALUES (:nombre, :username, :contrasena)"
...
async def actualizar_usuario(id: int, usuario: UsuariosIn):
    ...
    result = await db.execute(query, {"id":id, "nombre": usuario.nombre, "username": usuario.username, "contrasena": usuario.contrasena})
    if not result:
...
async def eliminar_usuario(id:int):
    ...
    if not result:
        raise HTTPException(status_code = 404, detail = "Usuario no encontrado")
    return {"message": f"Usuario {id} eliminado correctamente"}